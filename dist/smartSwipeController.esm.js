...
class SmartSwipeController {
  ...
  addEvents() {
    this._startEvent = e => {
      this._start.x = e.changedTouches ? e.changedTouches[0].screenX : e.screenX;
      this._start.y = e.changedTouches ? e.changedTouches[0].screenY : e.screenY;
    };
    this._endEvent = e => this._handler(e.changedTouches ? e.changedTouches[0].screenX : e.screenX, e.changedTouches ? e.changedTouches[0].screenY : e.screenY, e);
    this._events();
  }
  /**
  ...
  _handler(x, y, e) {
    const iX = x - this._start.x,
      iY = y - this._start.y;
    const normX = Math.abs(iX) - this.offset <= 0 ? 0 : Math.abs(iX) - this.offset,
